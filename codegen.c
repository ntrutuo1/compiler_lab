...
void genVariableAddress(Object* var) {
  // Sinh mã địa chỉ biến: tính khoảng cách cấp (static link distance)
  ...
  case OBJ_VARIABLE:
    ...
    break;
  case OBJ_PARAMETER:
    ownerScope = var->paramAttrs->scope;
    while (curScope != NULL && curScope != ownerScope) {
      curScope = curScope->outer;
      level++;
    }
    genLA(level, var->paramAttrs->localOffset);
    break;
  default:
    genHL();
    break;
...
void genVariableValue(Object* var) {
  // Sinh mã lấy giá trị biến: tính khoảng cách cấp
  Scope* curScope = symtab->currentScope;
  Scope* ownerScope = NULL;
  int level = 0;
  ...
  case OBJ_PARAMETER:
    ...
    genLV(level, var->paramAttrs->localOffset);
    break;
  default:
    genHL();
    break;
